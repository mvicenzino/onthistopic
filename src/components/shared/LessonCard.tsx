...
export function LessonCard({ title, duration, topic, speaker }: LessonCardProps) {
  return (
    ...
    >
      <div className="overflow-hidden rounded-xl bg-white border border-black/5 shadow-sm transition-all hover:shadow-md hover:border-black/10">
        <div className="relative">
          <ImagePlaceholder label={title} aspect="16/9" className="rounded-none" />
          <span className="absolute bottom-2 right-2 rounded bg-navy/90 px-2 py-1 font-mono text-xs text-white">
            {duration}
          </span>
        </div>
        <div className="p-4">
